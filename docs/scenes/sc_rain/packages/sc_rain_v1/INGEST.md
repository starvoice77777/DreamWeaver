...
来源：素材交付包 `sc_rain_v1`（2026-08，编排 **v8**）  
仓库路径：本目录  
...
| `scene/timeline.json`（内容包 **v8**） | 映射为 `timeline-contract-v1` → `rain_eaves_timeline_v8.json` |
...
- A01–A03 音量 / 进出场与空间关键帧时间点不变。
- A04：由两条 `wind_realistic` 循环短段改为单轨 **`wind_gust` oneshot**（约 3.7s），在 188s、458s 各触发一次；oneshot 窗口内仍有空间关键帧。
- 再生 fixture：`python server/scripts/build_rain_eaves_timeline_v8.py`

未入库：`audio/demo/`、`audio/alternatives/`（体积大，仅 WeChat 原包保留；验收可用原包）。

## 升级后动作

1. `alembic` 无需新迁移；启动 API 后走 `ensure_official_timelines`（rainEaves version < 8 会升级）。
2. 或调用非 production `POST /v1/admin/reseed-catalog`。
3. iOS：确认 Bundle 含 `rain_eaves_timeline_v8.json` 与 `wind_gust.wav`；冷启动或切远端后端后重开。
