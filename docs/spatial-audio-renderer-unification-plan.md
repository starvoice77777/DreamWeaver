...
- 前端定稿分支：codex/scene-catalog-creation-ui-refresh。
- 前端定稿提交：f6e05eb3a7800bb90c0568f567e8e7a1db6bf722。
- 前端基线：origin/integration/frontend-backend 的 d1c1935。
- 音频功能分支：codex/update-scene-presets。
- 音频功能提交：38b0e19，包含素材响度校准、圆盘边缘约 -40dB 的半径曲线和本计划初稿。
- 合并目标：integration/frontend-backend。
- 后端组合验证：61 项 pytest 通过，相关 Python Ruff 检查通过。
- 前端提交自身已通过完整无签名 iOS Debug 构建；当前 Windows 环境不能替代最终合并版本的 Xcode 构建与真机试听。

### 1.2 前端定稿对本计划的实际影响

- 官方目录从 14 个场景调整为 13 个；流光溢彩退役，但 emotionalFluid 仍保留为可复用渲染能力。
- 场景 UUID 与 API visual_style 原始字符串保持稳定；iOS 枚举改名为 himalaya、longRoad、wheatWave、flight、summerNight，并用显式 raw value 兼容旧数据。
- SpatialEditorView、TimelineEditorView、SpatialCanvasView 完成较大视觉和交互整理，但 SpatialEditorSource、SpatialTimelineViewModel、SceneCompositionMapper 的核心编辑语义没有被新的领域模型替换。
- 创建页仍由 SpatialTimelineViewModel 维护 currentTime 和约 60Hz 预览循环，仍通过 SceneCompositionMapper 在 timeline/composition 与编辑模型间转换。
- SharedComponents.swift 新增 SpatialDiskLayout 与 BreathingSpatialRings；创建页和播放页已经共享圆盘几何与视觉构件，但尚未共享音频渲染状态。
- SoundMixCircleEditor 仍是只读播放圆盘，从 AppState.currentScene.soundSources 读取启用声源，并使用 SpatialMixMapping.gain 仅调整节点视觉大小/透明度。
- AppState 新增冷启动场景背景与 palette 协调逻辑；现有 AppState → LocalPlaybackService 播放接线、私人 mix 对齐和官方 timeline 加载仍存在。
- “从已有场景创建”的真实入口仍是 CreateHubView → SpatialEditorSeed → SceneCompositionMapper；本次前端改动没有解决 SourceGroup/AudioClip 身份混用或播放语义不一致。

### 1.3 合并归属结论

- 保留前端：全部 Views/DesignSystem 布局、动画、手势、图标、共享圆盘视觉、13 场景名称/图片/枚举以及退役场景交互。
- 保留后端：最新正式音频、官方 timeline、仅洗头陪伴可使用人声、旧官方音轨清理、逐素材响度补偿、半径增益算法和测试。
- 手工拼合：MockDataService、SceneBackdropKind、server/app/services/seed_catalog.py、server/tests/test_content_api.py。
- AppState 自动合并后已人工复核：保留前端启动背景逻辑，同时保留官方人声作用域修复与音频播放接线。
- LocalPlaybackService 自动合并后已人工复核：保留 sceneID 限定的洗头本地 timeline 回退，以及 20 句人声每句切换各自 mastering compensation 的修复。

### 1.4 工作区保护

- docs/integration-remote-test-checklist.md 是用户已有修改，不属于本次提交；同步前已单文件暂存并记录 SHA-256，联调推送后必须原样恢复。
- docs/unpack.md、tmp/ 和用户文档均不属于本次提交，不得夹带、覆盖或清理。
...
前端文件属于高冲突区，只做适配层级的最小修改：

- DreamWeaver/Views/Create/SpatialTimelineModels.swift：将 SpatialEditorSource 拆分为 SourceGroup view model 与 AudioClip row model，或提供明确兼容适配层。
- DreamWeaver/Views/Create/SpatialTimelineViewModel.swift：停止维护第二套播放循环，改为驱动/订阅 SceneRenderer。
- DreamWeaver/Views/Create/SceneCompositionMapper.swift：作为 composition/timeline 到 SceneRenderPlan 的输入适配边界，不再直接发明播放语义。
- DreamWeaver/Views/Create/SpatialEditorSeed.swift：保留官方场景、私人场景与已有 composition 的导入入口，但导入结果必须进入同一 compiler。
- DreamWeaver/Views/Create/SpatialCanvasView.swift：消费 RendererState 的 group 位置，保留现有轨迹绘制、拖动、触觉和 BreathingSpatialRings。
- DreamWeaver/Views/Create/TimelineEditorView.swift：时间轴渲染 AudioClip，保留前端定稿的编排交互与可访问性。
- DreamWeaver/Views/Create/SpatialEditorView.swift：只负责定稿后的页面编排和控制接线，不放入新的音频调度逻辑。

### 10.4 播放页适配

- DreamWeaver/Views/Now/SoundMixCircleEditor.swift 只消费 SourceGroup UI state，保留只读交互定稿和 BreathingSpatialRings。
- DreamWeaver/Views/Components/SharedComponents.swift 中的 SpatialDiskLayout/BreathingSpatialRings 是共享 UI 构件，不承担 DSP 或 renderer 时钟职责。
- AppState 通过 group id 调用 renderer override；播放页当前为只读，若未来恢复拖动也不得绕过 renderer。
- onTimelineSourceChange 替换为 RendererState 订阅，或提供兼容适配层。
- RootTabView 的共享背景、NowView 的控件显隐/进度拖动、启动 palette 和底部导航逻辑不得被删除。
...
- 已合并前端定稿提交 f6e05eb3a7800bb90c0568f567e8e7a1db6bf722。
- 已合并音频功能提交 38b0e19。
- 已确认创建页核心模型/时钟未被替换，播放圆盘仍是 AppState 的只读声源投影。
- 已确认共享的只是 SpatialDiskLayout/BreathingSpatialRings UI 构件，不是渲染时钟或音频状态。
- 已按 1.3 节处理目录、枚举、MockData、AppState、LocalPlaybackService 和服务端种子的组合关系。
- 61 项后端测试和相关 Ruff 检查通过。

完成条件：

- 本次改动有明确提交边界，用户自己的工作区文件不夹带。
- 前端定稿提交自身的 Xcode 基线可编译；合并版本仍需在 macOS/Xcode 复验。
- 后端测试通过。
- 前端已确认交互定稿并停止修改本轮共享文件。
...
- 阶段 0 同步已完成；后续重构开始前再次确认前端没有恢复修改共享文件。
- 修改共享文件前通知前端暂停同文件。
- UI 冲突保留前端意图。
- 播放/Service 冲突保留后端意图。
