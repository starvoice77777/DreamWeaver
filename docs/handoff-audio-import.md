...
翻页入眠与耳畔轻语一样使用独立稳定场景 ID，仅在评审开关生效时作为新增目录项发布。
在 `development`、`local` 或 `test` 环境启用 `DW_ENABLE_HANDOFF_REVIEW_PRESETS=true` 后，
场景列表、详情和时间线会返回翻页场景、十一条轨道及 250 秒完整时间线；关闭开关后，目录项隐藏，
详情和时间线返回 404。生产和预发布环境即使设置开关也不会发布该场景。

后端沿用 App 的 `whisper` 分类、`snowStudy` 视觉和深蓝书房配色。首次启用会创建固定 ID 的官方记录，
重复读取和重新播种不会重复创建；缺失轨道会按 fixture 自动恢复。API 字段、数据库结构、iOS 代码及
本机 `.env` 均未改变，不需要 Alembic 迁移。联调时应重启开发 API 和客户端；发布前仍需完成 fixture
所列的 QC、许可证、循环接缝和实机试听审核。
